...
public class TimeUtil {
	...
	public static final Date getFirstDate() {
		Date date = new Date();
		Calendar ca = Calendar.getInstance();
	...
	public static final String getTodayMonthFinally() {
	    ...
	    shijian = format.format(cale.getTime());
	...
	public static String formatDatetime(String inDate) {
		...
		String result = inYear.trim() + "." + inMonth.trim() + "." + inDay.trim();
		if (inHour.trim().length() > 0) {
			result += " " + inHour.trim() + ":" + inMinute.trim() + ":" + inSecond.trim();
		} else {
			result += " 00:00:00";
		}
	...
	public static Date strToSysDate(String day, String hour, String minite) {
	...
	public static Object convert(Class type, Object value) {
		...
		throw new RenException("Could not convert " + value.getClass().getName() + " to " + type.getName());
	}
	...
	protected static Object convertToString(Class type, Object value) {
		...
		if (value instanceof Date) {
			try {
				dat = (Timestamp) value;
				if ((dat.getHours() == 0) && (dat.getMinutes() == 0) && (dat.getSeconds() == 0)) {
					ifdate = true;
				} else {
					ifdate = false;
				}
				str = (new SimpleDateFormat(Simple_DateTime_Format)).format(dat);
				if ((str.length() <= Simple_Date_Format_Length) || (ifdate)) { // 只包含日期。
					str = (new SimpleDateFormat(Simple_Date_Format)).format(dat);
				}
				return str;
			} catch (Exception e) {
				throw new RenException("Error converting Date to String");
			}
		}
	...
	protected static Object convertToDate(Class type, Object value) {
		...
		if (value instanceof String) {
			try {
				...
				throw new RenException("Error converting String to Date");
			}
		}
	...
	public static boolean compareDatetime(String smallDate, String middleDate, String bigDate) {
		boolean bRet = true;
		try {
	...
	public static boolean compareDatetime(Date smallDate, Date middleDate, Date bigDate) {
		boolean bRet = true;
		try {
			if (middleDate.compareTo(smallDate) < 0 || middleDate.compareTo(bigDate) > 0) {
				bRet = false;
			}
		} catch (Exception ex) {
			ex.printStackTrace();
	...
	public static boolean compareDatetime(String smallDate, Date middleDate, String bigDate) {
		boolean bRet = true;
		try {
			SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
			Date small = df.parse(smallDate);
	 ...
	 * @throws Exception
	 */
	public static String getBeginYearOfMonth(int year, int month) throws Exception {
		SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		Calendar c = Calendar.getInstance();
	...
	public static String getEndYearOfMonth(int year, int month) throws Exception {
		SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		Calendar c = Calendar.getInstance();
		c.set(year, month - 1, 1, 0, 0, 0);
	...
	public static List<String> getEverydayDate(String beginDate, String endDate){
		...
		try {
			...
			long offset = (dtEnd.getTime() - dtBegin.getTime()) / (24 * 60 * 60 * 1000);
	...
	public static String addMonthNew(int month) {
		Date dt = addMonth(new Date(), month);
		String str = formatDatetime(dt, "yyyy-MM");
		return str;
	}
	public static String addMonthNewSubtractMonth(Date date,int month) {
		Date dt = addMonth(date, month);
		String str = formatDatetime(dt, "yyyy-MM");
		return str;
	}

	public static Date getDateTimeByDay(String time) {
		String strRemindTime = TimeUtil.formatDatetime(new Date(), "yyyy-MM-dd") + " " + time;
	...
	public static Date getDateTimeByWeek(int week, String time) {
		...
		Date dt = c.getTime();
		String strRemindTime = TimeUtil.formatDatetime(dt, "yyyy-MM-dd") + " " + time;
	...
	public static String getOracleBeginDateTime(String beginDate) {
		String strBeginDate = "to_date('" + beginDate + " 00:00:00','yyyy-mm-dd hh24:mi:ss')";
		return strBeginDate;
	}

	public static String getOracleBeginDateTime(Date beginDate) {
		String strBeginDate = "to_date('" + formatDatetime(beginDate, "yyyy-MM-dd") + " 00:00:00','yyyy-mm-dd hh24:mi:ss')";
		return strBeginDate;
	}

	public static String getOracleEndDateTime(String endDate) {
		String strBeginDate = "to_date('" + endDate + " 23:59:59','yyyy-mm-dd hh24:mi:ss')";
		return strBeginDate;
	}

	public static String getOracleEndDateTime(Date endDate) {
		String strBeginDate = "to_date('" + formatDatetime(endDate, "yyyy-MM-dd") + " 23:59:59','yyyy-mm-dd hh24:mi:ss')";
		return strBeginDate;
	}
